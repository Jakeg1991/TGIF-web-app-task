...
window.onload = createAttendanceTable()
...
function createAttendanceTable() {
    for (i = 0; i < stats.engagedInfo.leastEngaged.length; i++) {
        var mytable = document.getElementById("leastEngaged");
        var row = document.createElement("tr");
        if (stats.engagedInfo.leastEngaged.middle_name == null) {
            middleName = " ";
        } else {
            var middleName = (stats.engagedInfo.leastEngaged.middle_name);
        }

        var name = (stats.engagedInfo.leastEngaged[i].first_name + middleName + stats.engagedInfo.leastEngaged[i].last_name)
        row.insertCell().innerHTML = name
        row.insertCell().innerHTML = stats.engagedInfo.leastEngaged[i].missed_votes;
        row.insertCell().innerHTML = stats.engagedInfo.leastEngaged[i].missed_votes_pct;
        mytable.append(row)
    }
    for (i = 0; i < stats.engagedInfo.mostEngaged.length; i++) {
        var mytable = document.getElementById("mostEngaged");
        var row = document.createElement("tr");
        if (stats.engagedInfo.mostEngaged.middle_name == null) {
            middleName = " ";
        } else {
            var middleName = (stats.engagedInfo.mostEngaged.middle_name);
        }

        var name = (stats.engagedInfo.mostEngaged[i].first_name + middleName + stats.engagedInfo.mostEngaged[i].last_name)
        row.insertCell().innerHTML = name
        row.insertCell().innerHTML = stats.engagedInfo.mostEngaged[i].missed_votes;
        row.insertCell().innerHTML = stats.engagedInfo.mostEngaged[i].missed_votes_pct;
        mytable.append(row)
    }
}